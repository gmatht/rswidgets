...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    widget_panel.append(&dd);

    let cb_grid = app.create_checkbutton("Grid")?;
    cb_grid.set_active(true);
    let cb_clone = cb_grid.clone();
    cb_grid.connect_toggled(move || {
        println!("  [widget] Grid toggle: active={}", cb_clone.is_active());
    })?;
    widget_panel.append(&cb_grid);

    let rb_a = app.create_radiobutton("Normal")?;
    let rb_b = app.create_radiobutton("Compact")?;
    rb_a.set_active(true);
    let ra = rb_a.clone();
    rb_a.connect_toggled(move || {
        if ra.is_active() {
            println!("  [widget] View: Normal");
        }
    })?;
    let rb = rb_b.clone();
    rb_b.connect_toggled(move || {
        if rb.is_active() {
            println!("  [widget] View: Compact");
        }
    })?;
    widget_panel.append(&rb_a);
    widget_panel.append(&rb_b);

    let tv = app.create_textview()?;
    tv.set_text("Interact with the widgets above.");
    tv.set_wrap_mode(0);
    tv.set_size_request(300, 24);
    widget_panel.append(&tv);

    vbox.append(&widget_panel);
    ...
    overlay.set_size_request(total_w as i32, total_h as i32);
    ...
    // ── Dialog keepalive (prevent NWG from destroying on drop) ─────
    let dialog_holder: Rc<RefCell<Vec<Box<dyn Any>>>> =
    ...
    let cbg = cb_grid.clone();
    let cvg = canvas.clone();
    connect_action!(sa_toggle_grid, {
        move || {
            cbg.set_active(!cbg.is_active());
            cvg.queue_redraw();
        }
    });
        ...
        move || {
            if let Ok(dialog) = af.create_dialog() {
                    ...
                    af.create_box(Orientation::Vertical, 4)
                {
                    ...
                    dialog.present();
                    dh.borrow_mut().push(Box::new(dialog));
                }
            }
        }
    });
        ...
        move || {
            if let Ok(dialog) = aa.create_dialog() {
                    ...
                    aa.create_box(Orientation::Vertical, 8)
                {
                    ...
                    dialog.connect_response(move |_| dlg.close()).ok();
                    dialog.present();
                    dh2.borrow_mut().push(Box::new(dialog));
                }
            }
        }
    });
    ...
    println!("View menu: Toggle Grid Lines");
    println!("Help menu: About");
    println!("Widget panel: DropDown, CheckButton, RadioButton, TextView");
